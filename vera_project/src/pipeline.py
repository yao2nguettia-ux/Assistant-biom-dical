...
class BiomedicalAssistant:
    ...
    def _get_verdict(self, question: str, context: str) -> dict:
        ...
        distribution = {ID2LABEL[i]: round(probs[i].item(), 4) for i in range(len(ID2LABEL))}
    ...
    def answer(self, question: str, context: str) -> dict:
        ...
        response["distribution"] = verdict_info["distribution"]
        response["contradiction_flag"] = contains_contradiction(explanation, verdict_info["verdict"])
        return response

    @staticmethod
    def _compare_verdicts(verdict_a: str, verdict_b: str) -> str:
        """Compare deux verdicts et renvoie 'accord', 'contradiction' ou 'partiel'."""
    ...
    def compare(self, question: str, context_a: str, context_b: str) -> dict:
        """Analyse la même question sur deux contextes (deux études) et
        indique si les deux verdicts s'accordent ou se contredisent.
        """
        score = self._ood_score(question)
        if score > self.config.oos_threshold:
            return {
        ...
        verdict_a = self._get_verdict(question, context_a)
        verdict_b = self._get_verdict(question, context_b)
        citation_a = self._cite_evidence(question, context_a)
        citation_b = self._cite_evidence(question, context_b)

        agreement = self._compare_verdicts(verdict_a["verdict"], verdict_b["verdict"])
        ...
        return {
            ...
            "a": {
                ...
                "confidence": verdict_a["confidence"],
                "citation": citation_a,
                "distribution": verdict_a["distribution"],
            },
            "b": {
                "verdict": verdict_b["verdict"],
                "confidence": verdict_b["confidence"],
                "citation": citation_b,
                "distribution": verdict_b["distribution"],
            },
        }
